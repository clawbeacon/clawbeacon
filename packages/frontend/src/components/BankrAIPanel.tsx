 ...
 * Provides AI Chat, Task Generator, and Daily Summarizer via Bankr LLM.
 */
...
type TabId = 'chat' | 'generate' | 'summary';
      ...
      <div className="flex-1 overflow-y-auto py-3 space-y-3 min-h-0">
        {chatHistory.map((msg, i) => (
          ...
          >
            ...
            {/* Bubble */}
            <div
              className={`rounded-xl px-3 py-2.5 text-xs leading-relaxed max-w-[85%] ${
                msg.role === 'assistant'
        ...
        {chatLoading && (
          <div className="flex gap-2.5 px-3">
            <div className="w-7 h-7 rounded-lg flex-shrink-0 flex items-center justify-center bg-red-500/20 border border-red-500/30 text-xs">
              🦞
            </div>
            <div className="bg-white/5 border border-white/5 rounded-xl rounded-tl-sm px-3 py-2.5">
              <div className="flex gap-1 items-center">
                {([0, 1, 2] as const).map((n) => (
                  <div
                    key={n}
                    className="w-1.5 h-1.5 rounded-full bg-red-400/60 animate-bounce"
        ...
        <div className="flex gap-2">
          ...
          <div className="flex flex-col gap-1.5">
            <button
              ...
              className="w-9 h-9 rounded-lg bg-red-500/20 border border-red-500/30 flex items-center justify-center hover:bg-red-500/30 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
            >
      ...
      <div className="p-3 space-y-3">
        ...
        {savedCount !== null && (
          <div className="flex items-center gap-2 p-2.5 bg-green-500/10 border border-green-500/20 rounded-lg text-xs text-green-400">
            ...
            <span>{savedCount} tasks saved to Kanban!</span>
          </div>
        )}
        ...
        {generatedTasks.length > 0 && (
          <div className="space-y-2">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-green-500/10 border border-green-500/30 text-green-400 text-xs font-semibold hover:bg-green-500/20 transition-all disabled:opacity-40"
            >
              {saving ? (
                <>
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
    ...
    return (
      <div className="h-full flex flex-col items-center py-4 bg-white/[0.02] gap-3">
        <button
          ...
          className="w-10 h-10 rounded-xl bg-red-500/10 border border-red-500/20 flex items-center justify-center hover:bg-red-500/20 transition-all hover:scale-105"
          title="Expand Bankr AI"
        >
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-3 border-b border-white/5 bg-white/[0.02] flex-shrink-0">
        <div className="flex items-center gap-2.5">
          <button
            onClick={onToggleCollapse}
            className="w-8 h-8 rounded-lg bg-red-500/10 border border-red-500/20 flex items-center justify-center hover:bg-red-500/20 transition-all hover:scale-105"
            title="Collapse Bankr AI"
          >
      ...
      <div className="flex-1 overflow-hidden">
        ...
        {activeTab === 'generate' && (
          <GenerateTab
            generatedTasks={ai.generatedTasks}
            generateLoading={ai.generateLoading}
            generateError={ai.generateError}
            generateTasks={ai.generateTasks}
            clearGeneratedTasks={ai.clearGeneratedTasks}
        ...
        {activeTab === 'summary' && (
          <SummaryTab
            tasks={tasks}
            summary={ai.summary}
            summaryLoading={ai.summaryLoading}
